#!/usr/bin/env bash
# Creates the concrete target application in the separate Forgejo working directory.
set -euo pipefail
...
[[ -d "${target}/.git" ]] || {
  echo "Forgejo working copy is missing: ${target}" >&2
  echo 'Complete Lab 02 first.' >&2
  exit 1
}
...
case "${TARGET_ARCH:-$(uname -m)}" in
  ...
  *) echo "Unsupported target architecture: ${TARGET_ARCH:-$(uname -m)}" >&2; exit 1 ;;
esac
...
resolve_reference() {
  ...
  [[ "$digest" =~ ^sha256:[0-9a-f]{64}$ ]] || {
    echo "Invalid digest for ${source}: ${digest}" >&2
    exit 1
  }
  printf '%s@%s' "$source" "$digest"
}

echo "Resolving image digests for linux/${target_arch} ..."
...
# Generated by scripts/prepare-target-application.sh.
schemaVersion: 1
component:
...
printf 'Target application prepared: %s\n' "$target"
printf 'Target platform: linux/%s\n' "$target_arch"
printf 'Next step: git -C %q status --short\n' "$target"
